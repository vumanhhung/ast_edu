...
const prefixWith = key => value => `${key}=${value}`;
const empty = () => "";
...
const runOrEmpty = (predicate, fn) => either(predicate, fn, empty);
...
const typedFormatter = runOrEmpty(hasField, either(isStringValue, compose(filterFormatter, quote, encodeValue), dateFormatter));
const join = ({ logic }) => `~${logic}~`;
const serialize = serializeFilters(filter => either(isCompositeFilterDescriptor, serialize, typedFormatter)(filter), join);
 ...
 * {% endplatform_content %}
 */
export const toDataSourceRequestString = (state) => (Object.keys(state)
    .map(rules(state))
